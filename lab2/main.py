...
def line_detection_vectorized(img, num_rhos=360, num_thetas=360, threshold=0.5, min_count=0):
    def _debug():
        if debug:
...
if __name__ == "__main__":
    debug = False
    line_detection_vectorized(cv2.imread(f"assets/sample-1.jpg"), threshold=0.60)
    line_detection_vectorized(cv2.imread(f"assets/sample-2.jpg"), num_rhos=720, num_thetas=720, threshold=0.30)
    line_detection_vectorized(cv2.imread(f"assets/sample-3.jpg"), min_count=200)
