...
def circle_detection(img, threshold=0.5, min_count=0):
    def _debug():
        if debug:
            cv2.imshow('res', edge_image)
            cv2.waitKey(-1)
    ...
    _, angle = cv2.cartToPolar(grad_x, grad_y)
    # (a, b): circle center
    ...
    a_idx, b_idx = centers[:, 0], centers[:, 1]
...
if __name__ == "__main__":
    ...
    circle_detection(cv2.imread(f"assets/sample-3.jpg"), threshold=0.50)
